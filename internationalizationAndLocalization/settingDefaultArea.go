...
func main() {
	fmt.Println("Locale 	在Linux和Solaris系统中可以通过locale -a命令列举所有支持的地区名，读者可以看到这些地区名的命名规范。对于BSD等系统，没有locale命令，但是地区信息存储在/usr/share/locale中。 ")
	...
	// 1 通过URL就可以很明显的识别
	// 2 用户可以通过域名很直观的知道将访问那种语言的站点
	// 3 在Go程序中实现非常的简单方便，通过一个map就可以实现
	// 4 有利于搜索引擎抓取，能够提高站点的SEO

	//  todo  SEO 搜索引擎优化
	fmt.Println("SEO（Search Engine Optimization）:汉译为搜索引擎优化。是一种方式:利用搜索引擎的规则提高网站在有关搜索引擎内的自然排名。目的是：为网站提供生态式的自我营销解决方案，让其在行业内占据领先地位，获得品牌收益；SEO包含站外SEO和站内SEO两方面；为了从搜索引擎中获得更多的免费流量，从网站结构、内容建设方案、用户互动传播、页面等角度进行合理规划，还会使搜索引擎中显示的网站相关信息对用户来说更具有吸引力。")
...
func localeDemo3() {
	...
	fmt.Println("当然在实际应用中，可能需要更加严格的判断来进行设置地区")
...
func localeDemo1() {
	...
	fmt.Println("通过域名设置Locale有如上所示的优点，但是我们一般开发Web应用的时候不会采用这种方式，因为首先域名成本比较高，开发一个Locale就需要一个域名，而且往往统一名称的域名不一定能申请的到，其次我们不愿意为每个站点去本地化一个配置，而更多的是采用url后面带参数的方式")
