...
* 2018.8.6
  ...
  *  对于枚举类型，我们可以通过C.enum_xxx来访问C语言中定义的enum xxx结构体类型
   ...
   *  CGO的一些Demo，在使用的时候，注意`#include <errno.h> ` 注意不要导错包了啊  
   *  因为C语言不支持返回多个结果，因此<errno.h>标准库提供了一个errno宏用于返回错误状态。我们可以近似地将errno看成一个线程安全的全局变量，可以用于记录最近一次错误的状态码.定义错误，给Go语言去使用是最好的
   * void函数的返回值
   * C调用Go导出的函数:没有导出成功   
   * CGO的内部的机制：todo  
...
* 2018.8.13
   * C语言的内存分配后就是稳定的，但是Go语言因为函数栈的动态的伸缩可能导致栈中的内存地址移动（GO与C内存模型的最大的差异），CGO 是Go和C之间的桥梁，它使二者在二进制接口层面实现了互通
   *  Go访问C内存:未发现和Go创建slice的异常，都是差不多，但是Go有限制，slice最多2G的内存空间，但是通过C来创建就没有那些的限制了
   * 通过C访问Go的内存，为了简化并高效的处理c语言传入go语言的内存的问题，cgo定义了规则，就是在cgo调用c语言函数返回前， cgo保证传入的go语言的内存在期间不会移动，c语言函数可以大胆的调用go语言得内存！
   * 但是这么好的技术不能够被滥用，加上C语言需要长时间运行，那么将会导致被他引用的Go语言的内存在c语言返回前不能够移动，就有可能导致Go内存栈对应的 goroutine 不能动态的伸缩栈内存，就会被阻塞，需要长时间运行的C语言函数，需要谨慎的处理Go语言的内存 
   * C长期持有Go指针对象:没有完成 麻痹 
   * Go是从一个固定的虚拟地址空间分配内存，而C语言的分配的内存则不能使用Go语言保留的虚拟内存空间，在CGO环境中，Go语言运行时默认会检查导出返回的内存是否是Go语言分配，如果是，就会抛出异常 
   * .......
